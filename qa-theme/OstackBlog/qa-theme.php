...
class qa_html_theme extends qa_html_theme_base
{
	 ...
	 * @param array $q_item
	 */
	public function q_item_read_more ($q_item)
	{
		$this->output(
			'<div class="q-list-item-read-more">',
			'<a href="' . $q_item['url'] . '" class="read-more-btn">' .  'Read More...</a>',
			'</div>'
		);
	}
	public function q_item_title($q_item)
	{
		$this->output(
			...
			'<span><a href="' . $q_item['url'] . '">' . $q_item['title'] . '</a></span>',
			'</div>'
		);
	}
	/**
	...
	public function attribution()
	{
		...
		$this->output(
			'<div class="qa-attribution">',
			'Blog Theme by <a href="http://www.OStack.cn">OStack.cn</a>',
			'</div>'
		);
	...
	public function q_list_content($q_item)
	{
		...
		if (!empty($q_item['raw']['content'])) {
			$text = qa_viewer_text($q_item['raw']['content'], $q_item['raw']['format'], array());
			$text = qa_shorten_string_line($text, 500);
			return qa_html($text).'...';
		}else{
